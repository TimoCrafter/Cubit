...
public class ConfigValues {
    ...
    public ConfigValues() {
        ...
        setupPath(disabledWorlds, pluginDisabledIn);
        setupPath(configVersion, 1.0D);
        setupPath(maxTaxAmount, 400.00D);

        setupPath(taxIncreasePerRegion, 10.0D);
        setupPath(firstRegionCosts, 256.0D);
        setupPath(percentageAtRegionSell, 0.5D);
        setupPath(buyupMembers, 20 * 24 * 60 * 60 * 1000);
        setupPath(buyupNoMembers, 35 * 24 * 60 * 60 * 1000);
        setupPath(buyupInfo, 10);
        setupPath(landBuyChunkBorders, Material.TORCH);
        setupPath(landSellChunkBorders, Material.REDSTONE_TORCH_ON);
        setupPath(setBiome, 300D);
        setupPath(landSave, 300D);
    ...
    private void setupPath(String path, Object value) {
