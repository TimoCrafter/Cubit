...
public class CMD_Biome_Set extends CubitCore

{
    ...
    public boolean execute(final CommandSender sender, final String[] args) {
        if (sender.hasPermission("cubit.biome.setbiome")) {
            ...
            if (args.length == 2) {
                this.setupLand.executorServiceCommands.submit(new Runnable() {
                    public void run() {
                        ...
                        if (Cubli.setBiome(player, regionName, Biome.valueOf(args[1].toUpperCase()))) {
                            moneyTransfer(player, null, costs);
                        }
                        if (isSpigot()) {
                            playEffect(player, Effect.COLOURED_DUST, 1);
                        }
                        CMD_Biome_Set.this.setupLand.executorServiceRegions.submit(new RegionSaveTask(CMD_Biome_Set.this.getWorldGuard(), null, world));
                    }
                });
                return true;
