...
public class CMD_Land_List extends CubitCore {
            ...
            setupLand.executorServiceCommands.submit(new Runnable() {
                public void run() {
                    if (args.length <= 2) {
                        ...
                        WorldGuardPlugin wg = setupLand.getCubitInstance().getHookManager().getWorldGuardManager().getWorldGuardPlugin();
                        ...
                        List<String> subList = getLandsOfPlayer(p).subList(pageNumb * 10, pageNumb * 10 + 10 > rgCount ? rgCount : pageNumb * 10 + 10);
                        sender.sendMessage(translate("messages.listLandPage", rm.getRegionCountOfPlayer(wg.wrapPlayer(p)), (pageNumb * 10 + 1), (pageNumb * 10 + 10)));
                        ...
                        for (String name : subList) {
                            ...
                            sender.sendMessage("§a" + counter + ". §6" + name + " §a(X,Z)   [" + posX + ", " + posZ + "]");
                            counter++;
                        }
                    }
                }
