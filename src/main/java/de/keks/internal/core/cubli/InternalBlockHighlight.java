...
public class InternalBlockHighlight implements Runnable {
    ...
    public int getY(World world, int x, int z) {
        int y;
        for (y = 255; y > 0; y--) {
            if ((world.getBlockAt(x, y, z).getType() != Material.AIR) && (world.getBlockAt(x, y, z).getType() != Material.ENDER_PORTAL_FRAME)
                    && (world.getBlockAt(x, y, z).getType() != Material.BEDROCK)) {
                if ((world.getBlockAt(x, y, z).getType() != Material.TORCH) && (world.getBlockAt(x, y, z).getType() != Material.REDSTONE_TORCH_ON)
                        && (world.getBlockAt(x, y, z).getType() != Material.DEAD_BUSH) && (world.getBlockAt(x, y, z).getType() != Material.SNOW)) {
                    break;
                }
                y--;
                break;
